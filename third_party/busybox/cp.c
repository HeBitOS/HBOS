...
#include "libbb.h"
/* HBOS: upstream's libcoreutils/coreutils.h just declares
 * cp_mv_stat2()/cp_mv_stat() (see libcoreutils/cp_mv_stat.c) -- those are
 * declared directly in libbb.h here instead, since this port doesn't
 * mirror upstream's coreutils/libcoreutils/ subdirectory split. */
...
#endif
	/* HBOS: this port's getopt32 doesn't implement the opt_complementary
	 * constraint string above ("rRd:Rd:" -- real upstream getopt32 uses
	 * it to make "-r" alias "-R" (and "-d"), see libbb.h's getopt32()
	 * comment). Without it, lowercase 'r' just gets its own bit position
	 * (11th letter in FILEUTILS_CP_OPTSTR "pdRfinlsLHarPvuTt:") instead of
	 * setting FILEUTILS_RECUR (hardcoded to uppercase 'R''s position, 1<<2)
	 * -- so plain "cp -r" silently failed to recurse. rm.c's own C code
	 * already ORs both letters' bits together explicitly for the same
	 * reason; cp.c didn't, so do it here instead of changing getopt32
	 * itself (a same-letter-different-case aliasing rule there would
	 * incorrectly merge this same opts string's -T/-t, which mean
	 * different things). */
	if (flags & (1 << 11)) flags |= FILEUTILS_RECUR;
	argc -= optind;
	argv += optind;
