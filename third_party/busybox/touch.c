...
# define timebuf        ((struct timespec*)NULL)
	...
	/* HBOS: upstream guards this whole definition behind #if
	 * ENABLE_LONG_OPTS, but the getopt32long() call below references
	 * touch_longopts unconditionally -- fine upstream (real long-option
	 * support there means the two always agree), but this port's
	 * ENABLE_LONG_OPTS is 0 while getopt32long()'s call site isn't
	 * conditional. Since this port's getopt32long() ignores its
	 * long-option-table argument entirely anyway (see libbb.h), just
	 * define it unconditionally instead of touching the call site. */
