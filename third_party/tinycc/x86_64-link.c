...
/* HBOS: real Linux uses 0x400000 as the default non-PIE load address, but
 * HBOS's own ELF loader (src/elf.c's elf64_load_and_spawn) and every other
 * .hax app (via user.ld) use 0x1000000000 — low addresses like 0x400000
 * aren't mapped/available in a freshly created user address space
 * (vmm_create_address_space()), so TCC-compiled output failed to load
 * ("segment map failed") until this was changed to match. */
#define ELF_START_ADDR 0x1000000000
