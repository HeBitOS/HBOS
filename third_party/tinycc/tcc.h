...
/* HBOS: this auto-detection triggers for us too (we build on an x86_64 host
 * targeting TCC_TARGET_X86_64), pulling in tccrun.c's whole -run/JIT path
 * (host dlopen/mmap(PROT_EXEC)-based in-process execution) — HBOS's port
 * deliberately doesn't support that (compile to a file, then run it
 * separately via elf64_load_and_spawn; see config.h and
 * third_party/tinycc/README.md). Force it back off. */
#ifdef HBOS_TCC_CONFIG_H
# undef TCC_IS_NATIVE
#endif
