...
#else
    /* HBOS: skip the traditional glibc-style crt1.o/crti.o search — HBOS has
     * no such files (no "/usr/lib" tree at all). tcc.c's HBOS patch
     * ("auto-link our own crt0+libc runtime bundle") appends
     * /system/lib/hbos_runtime.o instead, which already contains crt0.o's
     * _start plus all of HBOS's user libc — see third_party/tinycc/README.md
     * "Linking user output". */
#endif
}
...
#else
    /* HBOS: no crtn.o either — see tccelf_add_crtbegin() above. */
#endif
}
...
# endif
        ...
        /* HBOS: no "libc.so"/"-lc" search — see tccelf_add_crtbegin() above,
         * same reasoning: our own auto-linked /system/lib/hbos_runtime.o
         * bundle already provides every libc symbol directly as object code. */
#ifdef TCC_LIBGCC
        if (!s1->static_link) {
            if (TCC_LIBGCC[0] == '/')
