...
/* HBOS: register with the .hax auto-discovery system (see genhax.py /
 * app/include/hax.h) without pulling in that header's full GUI/SDK surface
 * here — tcc is TUI-only and doesn't touch any of it. Kept minimal and
 * self-contained so it can't conflict with tcc.h's own careful stdint/
 * stddef setup below. */
typedef struct {
...
static const hbos_hax_meta_t hbos_tcc_hax_meta = {
    0x4D584148u, HBOS_HAX_KIND_TUI, "tcc", "TinyCC C compiler"
};

#include "tcc.h"

/* HBOS: floating-point literal parsing (tccpp.c's number lexer) needs real
 * strtod/strtof/strtold/ldexpl. HBOS's general user libc is built without
 * SSE2 (see Makefile USER_CFLAGS) since no other .hax app needs FPU, so
 * these live here instead — compiled as part of tcc.c's own translation
 * unit, which (uniquely, see Makefile TCC_CFLAGS) keeps SSE2 enabled. Not
 * IEEE-754-exact for extreme edge cases, but correct for normal literals —
 * good enough for a hobby-OS C compiler front end. Must appear before
 * "libtcc.c" (-> tccpp.c) is included below, since nothing else declares
 * strtod/ldexpl and an implicit declaration wouldn't match their real
 * (floating-point-returning) signature. */
static long double hbos_strtold_impl(const char *s, char **endptr) {
    while (*s == ' ' || *s == '\t') s++;
    int neg = 0;
    ...
    if (*s == '.') {
        ...
        while (*s >= '0' && *s <= '9') { val += (*s - '0') * frac; frac *= 0.1L; s++; }
    }
    if (*s == 'e' || *s == 'E') {
        const char *save = s;
        s++;
        ...
        if (*s >= '0' && *s <= '9') {
            ...
            long double scale = 1;
            for (int i = 0; i < exp; i++) scale *= 10;
            val = esign < 0 ? val / scale : val * scale;
        } else {
            s = save; /* no digits after e/E — not part of the number */
        }
    }
    if (endptr) *endptr = (char *)s;
    return neg ? -val : val;
}
double strtod(const char *s, char **endptr) { return (double)hbos_strtold_impl(s, endptr); }
float strtof(const char *s, char **endptr) { return (float)hbos_strtold_impl(s, endptr); }
long double strtold(const char *s, char **endptr) { return hbos_strtold_impl(s, endptr); }

long double ldexpl(long double x, int exp) {
    if (exp > 0) { while (exp-- > 0) x *= 2.0L; }
    else { while (exp++ < 0) x /= 2.0L; }
    return x;
}
#if ONE_SOURCE
# include "libtcc.c"
...
redo:
    ...
    /* HBOS: auto-link our own crt0+libc runtime bundle so `tcc foo.c -o foo`
     * works standalone, matching plain gcc/clang ergonomics — real programs
     * need _start/printf/malloc/etc. from *somewhere*, and HBOS has no
     * persistent /usr/lib-style tree of crt objects for TCC to find on its
     * own (see third_party/tinycc/README.md "Linking user output"). */
    if (0 == ret && !s->nostdlib && !s->option_r
        && s->output_type == TCC_OUTPUT_EXE) {
        ret = tcc_add_file(s, "/system/lib/hbos_runtime.o");
    }
