...
/* HBOS: tcc_add_crt() removed — it had zero remaining callers once
 * tccelf_add_crtbegin()/tccelf_add_crtend() were patched to skip the
 * crt1.o/crti.o/crtn.o search (see tccelf.c and README.md "Linking user
 * output"). All of its original callers lived in #if branches for other
 * platforms this build never compiles (OpenBSD/FreeBSD/NetBSD/Android). */
