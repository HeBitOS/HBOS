...
| quickjs JS 运行时 | 可用初版 | vendored quickjs + musl libm 子集编译为 `js.hax`（ring3，真实 SSE2 浮点）；`run js -e <code>` / `js <file>` 执行脚本，`-t` 内置 13 项自测（算术/Math/字符串/数组/JSON/递归/异常/Date/dtoa）输出 `LINUX_JS: PASS`。已修复初始栈 ABI 对齐（main 入口 rsp≡8 mod 16），SSE2 应用不再因 movdqa 错位 GPF。`-w <file>` 模式把 `document.title` 与脚本输出写入文件供调用方读回 |
| 浏览器 `<script>` 执行 | 可用初版 | 轻量浏览器提取内联 `<script>`（无 src）→ **每个脚本独立文件**（js_pN.js）→ `js.hax -w` 多文件逐个 eval（一个 SyntaxError 不影响其他）→ title 更新窗口标题、console/print/document.write 输出追加为 BRK_CODE 渲染块；`browser_exec_scripts_core` 与 `jspage` 命令共用同一实现。bilibili 首页实测：HTTPS 加载 HTTP 200，10 个内联脚本全部执行，Vite `import.meta` 模块语法脚本被隔离。DOM stub（JS 定义）：惰性元素对象（innerHTML/classList/addEventListener 等）、`document.querySelector/getElementById/createElement/body`、`navigator.userAgent`、`location.href`、`window.setTimeout/fetch/XMLHttpRequest` 等。浏览器新增 `file://` 本地页面支持。v1 限制：不执行外链脚本、无 DOM 树（元素是惰性空壳）、无执行超时 |
