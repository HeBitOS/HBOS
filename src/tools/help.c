...
static void print_cmd_help(const char *name) {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
        if (strcmp(list[i]->name, name) == 0) {
            ...
            console_puts("\n  \x1b[36mDesc:\x1b[0m  "); console_puts(list[i]->description);
            console_puts("\n  \x1b[36mUsage:\x1b[0m "); console_puts(list[i]->usage);
            console_puts("\n");
            return;
        }
    }
    console_puts("\x1b[31mUnknown command:\x1b[0m "); console_puts(name); console_puts("\n");
}

static void cmd_help(int argc, char **argv) {
    if (argc > 1) { print_cmd_help(argv[1]); return; }
    console_puts("\n\x1b[33mWelcome to HBOS interactive help mode\x1b[0m\n");
    console_puts("Type a \x1b[36mcommand name\x1b[0m for details.\n");
    console_puts("Type '\x1b[36mlist\x1b[0m' to see all commands.\n");
    console_puts("Type '\x1b[36mexit\x1b[0m' or '\x1b[36mquit\x1b[0m' to leave.\n\n");
